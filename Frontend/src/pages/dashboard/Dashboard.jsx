        ...
        return [
            {
                ...
                note: 'Purchase requests waiting',
                tone: 'blue',
            },
            {
                label: 'Approved PR',
                value: approvedRequests.length,
                note: 'Ready for purchase order',
                tone: 'emerald',
            },
            {
                label: 'PO for Approval',
                value: draftedOrders.length,
                note: 'Draft purchase orders',
                tone: 'amber',
            },
            {
                label: 'Approved PO',
                value: approvedOrders.length,
                note: 'Released orders',
                tone: 'violet',
            },
            {
                label: 'Pending Items',
                value: pendingMaterials.length,
                note: 'Material decisions open',
                tone: 'rose',
            },
            {
                label: 'Masters',
                value: projects.length + vendors.length,
                note: `${projects.length} projects, ${vendors.length} vendors`,
                tone: 'slate',
            },
        ]
    }, [dashboardData])
            ...
            <div className='mb-6 flex flex-col justify-between gap-4 border-b border-slate-200 pb-5 lg:flex-row lg:items-end'>
                <div>
                    ...
                    <p className='mt-2 max-w-2xl text-sm text-slate-600'>
                        Live overview from purchase requests, purchase orders, projects, and vendors.
                    </p>
                </div>
                ...
                <div className='flex flex-wrap items-center gap-2'>
                    <div className='flex items-center gap-2 rounded-md border border-slate-200 bg-white px-3 py-2 text-sm text-slate-600 shadow-sm'>
                        ...
                        {lastUpdated && !isLoading && (
                            <span className='hidden border-l border-slate-200 pl-2 text-xs text-slate-400 sm:inline'>
                                {lastUpdated}
                            </span>
                        )}
                    </div>
                ...
                <aside className='rounded-md border border-slate-200 bg-white p-5 shadow-sm'>
                    ...
                    <div className='mt-4 space-y-4'>
                        <div className='flex items-center justify-between border-b border-slate-100 pb-3'>
                            <span className='text-sm text-slate-600'>Total PR</span>
                            <strong className='text-sm text-slate-950'>{dashboardData.purchaseRequests.length}</strong>
                        </div>
                        <div className='flex items-center justify-between border-b border-slate-100 pb-3'>
                            <span className='text-sm text-slate-600'>Total PO</span>
                            <strong className='text-sm text-slate-950'>
                                {dashboardData.draftedOrders.length + dashboardData.approvedOrders.length}
                            </strong>
                        </div>
                        <div className='flex items-center justify-between border-b border-slate-100 pb-3'>
                            <span className='text-sm text-slate-600'>Active Projects</span>
                            <strong className='text-sm text-slate-950'>{dashboardData.projects.length}</strong>
                        </div>
                        <div className='flex items-center justify-between'>
                            <span className='text-sm text-slate-600'>Registered Vendors</span>
                            <strong className='text-sm text-slate-950'>{dashboardData.vendors.length}</strong>
                        </div>
                    </div>
                </aside>
            </div>
