            ...
            <div className="">
                <div className="bg-white w-[80%] h-[90%] rounded-xl p-6">
                    <div className="flex justify-between items-center mb-5">
                        <h2 className="text-base font-bold">Approved Purchase Requests :-</h2>
                        <Button lable="Create PO Manually" onClick={() => { setSelectedPR(null); setIsOpenManualPOForm(true); }} className="flex items-center justify-center gap-3 py-3 px-6 bg-indigo-600 text-white text-xs font-semibold rounded-xl hover:bg-indigo-800 hover:cursor-pointer transition" icon={<i className=" fa-solid fa-pen-to-square text-ms"></i>} />
                    </div>
                    ...
                    <div className="h-[85%] overflow-auto rounded-t-lg">
                        <table className="w-full">
                            <thead className="bg-[#4b5ea3] text-white text-xs">
                                <tr>
                                    <th className="p-3 text-center">PR No</th>
                                    <th className="p-3 text-center">PR Date</th>
                                    <th className="p-3 text-center">Project</th>
                                    <th className="p-3 text-center">PR Status</th>
                                    <th className="p-3 text-center">Deliver Before</th>
                            ...
                            <tbody>
                                {approvedPRs.length === 0 ? (
                                    <tr>
                                        <td colSpan="6" className="text-center py-6 text-gray-400">No approved requests found</td>
                                    </tr>
                                ) : (
                                    approvedPRs.map((pr) => (
                                        <tr className="border border-b-gray-300 border-x-gray-100 text-xs" key={pr.request_id}>
